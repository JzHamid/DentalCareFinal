...
<body class="overflow-hidden vh-100">
    ...
    // Remaining balance
    $remainingBalance = $totalAfterDiscount - $appointment->total_paid;
    @endphp
    ...
    <div class="modal fade" id="appointmentModal-{{ $appointment->id }}" tabindex="-1" aria-labelledby="appointmentModalLabel-{{ $appointment->id }}" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content" id="appointment-printable-{{ $appointment->id }}">
                        ...
                        <div id="payment-form" class="no-print"> <!-- Payment Form -->
                            <div class="card border-0 shadow-sm no-print">
                                <div class="card-body">
                                    ...
                                    <form action="{{ route('store.payment', $appointment->id) }}" method="POST" onsubmit="return validatePayment()">
                                        @csrf
                                        <div class="mb-3">
                                            <label for="amount_paid-{{ $appointment->id }}" class="form-label">Payment Amount</label>
                                            <input type="number" step="0.01" class="form-control" name="amount_paid" id="amount_paid-{{ $appointment->id }}" required
                                                max="{{ $remainingBalance }}">
                                            <small class="text-muted">Remaining Balance: ₱{{ number_format($remainingBalance, 2) }}</small>
                                            <div id="amount_error-{{ $appointment->id }}" class="text-danger"></div>
                                        </div>
                                        <div class="mb-3">
                                            <label for="payment_type-{{ $appointment->id }}" class="form-label">Payment Type</label>
                                            <select class="form-select" name="payment_type" id="payment_type-{{ $appointment->id }}" onchange="updatePaymentAmount('{{ $appointment->id }}', {{ $remainingBalance }})">
                                                <option value="" selected disabled>Choose a payment option</option>
                                                <option value="full">Full</option>
                                                <option value="installment">Partial</option>
                                            </select>
                                        </div>
                ...
                <script>
                    function updatePaymentAmount(appointmentId, remainingBalance) {
                        const paymentType = document.getElementById(`payment_type-${appointmentId}`).value;
                        const amountPaidInput = document.getElementById(`amount_paid-${appointmentId}`);
                        const amountError = document.getElementById(`amount_error-${appointmentId}`);

                        if (paymentType === 'full') {
                            amountPaidInput.value = remainingBalance.toFixed(2);
                    ...
                    function validatePayment() {
                        ...
                        if (!paymentType) {
                            amountError.textContent = 'Please select a payment type';
                            return false;
                        }
                        ...
                        if (paymentType === 'installment' && amountPaid < 1000) {
                            amountError.textContent = 'Minimum amount for partial payment is ₱1000';
                            return false;
                        }
