...
class AdminController extends Controller
{
    ...
    public function appointment_status(Request $request, $id)
    {
        ...
        if ($request->status == 'Deny') {
            ...
            $email = $appointment->email ?? $appointment->user->email ?? null;
    
            if ($email) {
                Mail::to($email)->send(new DenialReasonMail($appointment, $reason));
            } else {
                // Optional: Log or handle if there's no recipient email
                logger()->warning("No email found for appointment ID: $id");
            }
        }
